...
3. If `public/index.html` or a route used by the page changed: start the server (`npm run dev`) and run `npm run e2e`. Or click through with the Playwright MCP: open `/`, add a run, confirm it appears.
...
- env contract: ok|gap (which variable)
